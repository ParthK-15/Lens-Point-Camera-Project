module.exports = [
  {
    "title": "Canon EF-S 18-135mm Lens",
    "slug": "canon-ef-s-18-135mm-lens",
    "company": "Canon",
    "price": 32995,
    "image": "/Assets/canon ef-s 1.png",
    "sliderImages": [
      "/Assets/canon ef-s 1.png",
      "/Assets/canon ef-s 2.png",
      "/Assets/canon ef-s 3.png",
      "/Assets/canon ef-s 4.png",
      "/Assets/canon ef-s 5.png"
    ],
    "description": "A versatile all-in-one zoom lens ideal for travel, portraits, and everyday photography with smooth autofocus and image stabilization.",
    "features": [
      "18–135mm zoom range for versatile shooting from wide-angle to telephoto.",
      "f/3.5–5.6 aperture for balanced performance in different lighting conditions.",
      "Optical Image Stabilization reduces blur from camera shake.",
      "STM motor enables smooth and silent autofocus for photos and videos.",
      "Lightweight and compact design for easy portability.",
      "High-quality optics deliver sharp images with good contrast."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "18–135mm"
      },
      {
        "key": "Aperture",
        "value": "f/3.5–5.6"
      },
      {
        "key": "Lens Type",
        "value": "Zoom"
      },
      {
        "key": "Mount",
        "value": "Canon EF-S"
      },
      {
        "key": "Stabilization",
        "value": "Yes"
      },
      {
        "key": "Weight",
        "value": "480g"
      }
    ],
    "detailSections": [
      {
        "title": "Versatile Zoom",
        "description": "The 18–135mm zoom range allows you to capture wide-angle to telephoto shots with ease.",
        "image": "/Assets/canon ef-s 1.png"
      },
      {
        "title": "Image Stabilization",
        "description": "Built-in stabilization reduces blur and helps produce sharper images in low-light conditions.",
        "image": "/Assets/canon ef-s 2.png"
      },
      {
        "title": "STM Autofocus",
        "description": "STM motor provides smooth and quiet autofocus for both photography and video.",
        "image": "/Assets/canon ef-s 3.png"
      }
    ],
    "designSections": [
      {
        "title": "Versatile Build",
        "description": "Designed for multiple shooting scenarios, this lens adapts easily from wide-angle to telephoto use, enhancing overall shooting convenience.",
        "image": "/Assets/canon ef-s 4.png"
      },
      {
        "title": "Ergonomic Handling",
        "description": "Designed for comfortable use, the lens provides a secure grip and smooth zoom control, allowing easy operation during extended shooting sessions.",
        "image": "/Assets/canon ef-s 5.png"
      }
    ],
    "connectivity": [
      {
        "title": "Travel",
        "description": "Perfect all-in-one lens for travel photography."
      },
      {
        "title": "Portrait",
        "description": "Creates pleasing background blur at higher focal lengths."
      },
      {
        "title": "Landscape",
        "description": "Wide-angle range suitable for scenic shots."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/ZwLNmoaMoPo",
    "category": "lens",
    "subCategory": "zoom"
  },
  {
    "title": "Canon EF 100mm f/2.8 Macro USM",
    "slug": "canon-100-2-8",
    "company": "Canon",
    "price": 49999,
    "image": "/Assets/cannon 100mm2-81 (1).png",
    "sliderImages": [
      "/Assets/cannon 100mm2-81 (1).png",
      "/Assets/cannon 100mm2-81 (2).png",
      "/Assets/cannon 100mm2-81 (3).png",
      "/Assets/cannon 100mm2-81 (4).png",
      "/Assets/cannon 100mm2-81 (5).png"
    ],
    "description": "A high-performance macro lens designed for close-up photography with exceptional detail and sharpness.",
    "features": [
      "True 1:1 macro magnification.",
      "Sharp optics for detailed close-up shots.",
      "USM autofocus for fast and silent focusing.",
      "Ideal for macro and portrait photography.",
      "Lightweight and easy to handle."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "100mm"
      },
      {
        "key": "Aperture",
        "value": "f/2.8"
      },
      {
        "key": "Mount",
        "value": "Canon EF"
      },
      {
        "key": "Magnification",
        "value": "1:1 Macro"
      },
      {
        "key": "Weight",
        "value": "600g"
      }
    ],
    "detailSections": [
      {
        "title": "Macro Capability",
        "description": "Capture tiny subjects with life-size magnification and incredible detail.",
        "image": "/Assets/cannon 100mm2-81 (2).png"
      },
      {
        "title": "Sharp Optics",
        "description": "Delivers excellent clarity and sharpness across the frame.",
        "image": "/Assets/cannon 100mm2-81 (3).png"
      },
      {
        "title": "USM Autofocus",
        "description": "Fast and quiet autofocus for precise subject capture.",
        "image": "/Assets/cannon 100mm2-81 (4).png"
      }
    ],
    "designSections": [
      {
        "title": "Compact Build",
        "description": "Lightweight and easy to carry for everyday use.",
        "image": "/Assets/cannon 100mm2-81 (5).png"
      },
      {
        "title": "Durable Construction",
        "description": "Built with high-quality materials for long-term reliability.",
        "image": "/Assets/cannon 100mm2-81 (2).png"
      }
    ],
    "connectivity": [
      {
        "title": "Macro",
        "description": "Perfect for insects, flowers, and product photography."
      },
      {
        "title": "Portrait",
        "description": "Sharp subject with smooth background blur."
      },
      {
        "title": "Detail Shots",
        "description": "Capture textures and fine details with precision."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/nGwwSObE5c0",
    "category": "lens",
    "subCategory": "prime"
  },
  {
    "title": "Canon EF 100-400mm f/4.5-5.6L IS II USM",
    "slug": "canon-100-400",
    "company": "Canon",
    "price": 189999,
    "image": "/Assets/canon 100-4001.png",
    "sliderImages": [
      "/Assets/canon 100-4001.png",
      "/Assets/canon 100-4002.png",
      "/Assets/canon 100-4003.png",
      "/Assets/canon 100-4004.png",
      "/Assets/canon 100-4005.png"
    ],
    "description": "A premium L-series super-telephoto lens designed for wildlife, sports, and long-distance photography with exceptional clarity.",
    "features": [
      "100–400mm super-telephoto zoom range.",
      "Advanced 4-stop Image Stabilization.",
      "Ring-type USM for fast and silent autofocus.",
      "L-series professional build quality.",
      "Dust and weather-resistant design.",
      "Sharp optics with minimal chromatic aberration."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "100–400mm"
      },
      {
        "key": "Aperture",
        "value": "f/4.5–5.6"
      },
      {
        "key": "Lens Type",
        "value": "Zoom"
      },
      {
        "key": "Mount",
        "value": "Canon EF"
      },
      {
        "key": "Stabilization",
        "value": "Yes"
      },
      {
        "key": "Weight",
        "value": "1640g"
      }
    ],
    "detailSections": [
      {
        "title": "Super Telephoto Zoom",
        "description": "The 100–400mm range allows capturing distant subjects with incredible detail.",
        "image": "/Assets/canon 100-4001.png"
      },
      {
        "title": "Image Stabilization",
        "description": "Advanced stabilization reduces camera shake for sharper handheld shots.",
        "image": "/Assets/canon 100-4002.png"
      },
      {
        "title": "USM Autofocus",
        "description": "Fast and precise autofocus ensures accurate subject tracking.",
        "image": "/Assets/canon 100-4003.png"
      }
    ],
    "designSections": [
      {
        "title": "Professional Build",
        "description": "L-series construction ensures durability and reliability in demanding environments.",
        "image": "/Assets/canon 100-4004.png"
      },
      {
        "title": "Weather Sealing",
        "description": "Dust and moisture resistance make it ideal for outdoor photography.",
        "image": "/Assets/canon 100-4005.png"
      }
    ],
    "connectivity": [
      {
        "title": "Wildlife",
        "description": "Perfect for capturing distant animals with clarity."
      },
      {
        "title": "Sports",
        "description": "Ideal for fast action shots from long distances."
      },
      {
        "title": "Nature",
        "description": "Great for capturing landscapes and detailed scenes."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/L57ok2pjqyg",
    "category": "lens",
    "subCategory": "zoom"
  },
  {
    "title": "Canon EF 24-105mm f/4L IS II",
    "slug": "canon-24-105",
    "company": "Canon",
    "price": 89995,
    "image": "/Assets/Canon EF 24-105mm f4L IS II51.png",
    "sliderImages": [
      "/Assets/Canon EF 24-105mm f4L IS II51.png",
      "/Assets/Canon EF 24-105mm f4L IS II52.png",
      "/Assets/Canon EF 24-105mm f4L IS II53.png",
      "/Assets/Canon EF 24-105mm f4L IS II54.png",
      "/Assets/Canon EF 24-105mm f4L IS II55.png"
    ],
    "description": "A professional L-series zoom lens with constant f/4 aperture, delivering excellent image quality and versatile performance.",
    "features": [
      "24–105mm zoom range for wide to telephoto shooting.",
      "Constant f/4 aperture for consistent exposure.",
      "Advanced Image Stabilization for sharp images.",
      "USM autofocus for fast and silent focusing.",
      "Professional L-series build quality.",
      "High-quality optics with minimal distortion."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "24–105mm"
      },
      {
        "key": "Aperture",
        "value": "f/4"
      },
      {
        "key": "Lens Type",
        "value": "Zoom"
      },
      {
        "key": "Mount",
        "value": "Canon EF"
      },
      {
        "key": "Stabilization",
        "value": "Yes"
      },
      {
        "key": "Weight",
        "value": "795g"
      }
    ],
    "detailSections": [
      {
        "title": "Versatile Zoom",
        "description": "The 24–105mm zoom range allows you to capture wide-angle to telephoto shots with ease.",
        "image": "/Assets/Canon EF 24-105mm f4L IS II52.png"
      },
      {
        "title": "Image Stabilization",
        "description": "Advanced stabilization reduces blur and ensures sharper images in low light.",
        "image": "/Assets/Canon EF 24-105mm f4L IS II53.png"
      },
      {
        "title": "USM Autofocus",
        "description": "USM motor provides fast, accurate, and silent autofocus performance.",
        "image": "/Assets/Canon EF 24-105mm f4L IS II54.png"
      }
    ],
    "designSections": [
      {
        "title": "Professional Build",
        "description": "L-series construction ensures durability and reliability in demanding conditions.",
        "image": "/Assets/Canon EF 24-105mm f4L IS II55.png"
      },
      {
        "title": "Ergonomic Handling",
        "description": "Smooth zoom and focus rings provide precise control and comfortable handling.",
        "image": "/Assets/Canon EF 24-105mm f4L IS II51.png"
      }
    ],
    "connectivity": [
      {
        "title": "Travel",
        "description": "Ideal for capturing a wide range of scenes while traveling."
      },
      {
        "title": "Portrait",
        "description": "Produces sharp subjects with pleasing background blur."
      },
      {
        "title": "Landscape",
        "description": "Wide-angle capability for capturing scenic views."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/2ptnyjhw7tU",
    "category": "lens",
    "subCategory": "zoom"
  },
  {
    "title": "Canon EF 50mm f/1.8 STM",
    "slug": "canon-50-18",
    "company": "Canon",
    "price": 9999,
    "image": "/Assets/canon50-18-1.png",
    "sliderImages": [
      "/Assets/canon50-18-1.png",
      "/Assets/canon50-18-2.png",
      "/Assets/canon50-18-3.png",
      "/Assets/canon50-18-4.png",
      "/Assets/canon50-18-5.png"
    ],
    "description": "A compact prime lens known for its sharp image quality and beautiful background blur, perfect for portraits.",
    "features": [
      "50mm prime lens for natural perspective.",
      "Wide f/1.8 aperture for low-light shooting.",
      "Creates beautiful background blur (bokeh).",
      "STM motor for smooth and silent autofocus.",
      "Compact and lightweight design."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "50mm"
      },
      {
        "key": "Aperture",
        "value": "f/1.8"
      },
      {
        "key": "Lens Type",
        "value": "Prime"
      },
      {
        "key": "Mount",
        "value": "Canon EF"
      },
      {
        "key": "Weight",
        "value": "160g"
      }
    ],
    "detailSections": [
      {
        "title": "Wide Aperture",
        "description": "The f/1.8 aperture allows excellent low-light performance and shallow depth of field.",
        "image": "/Assets/canon50-18-2.png"
      },
      {
        "title": "Sharp Optics",
        "description": "Produces crisp and detailed images with natural colors.",
        "image": "/Assets/canon50-18-3.png"
      },
      {
        "title": "STM Autofocus",
        "description": "Smooth and quiet focusing ideal for both photos and video.",
        "image": "/Assets/canon50-18-4.png"
      }
    ],
    "designSections": [
      {
        "title": "Compact Build",
        "description": "Lightweight and portable, perfect for everyday use.",
        "image": "/Assets/canon50-18-5.png"
      },
      {
        "title": "Durable Design",
        "description": "Built with quality materials for reliable performance.",
        "image": "/Assets/canon50-18-1.png"
      }
    ],
    "connectivity": [
      {
        "title": "Travel",
        "description": "Ideal for capturing a wide range of scenes while traveling."
      },
      {
        "title": "Portrait",
        "description": "Produces sharp subjects with pleasing background blur."
      },
      {
        "title": "Landscape",
        "description": "Wide-angle capability for capturing scenic views."
      },
      {
        "title": "Portrait",
        "description": "Perfect for capturing sharp subjects with blurred background."
      },
      {
        "title": "Low Light",
        "description": "Excellent performance in dim lighting conditions."
      },
      {
        "title": "Street",
        "description": "Great for everyday and street photography."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/PoFsXaXKetI",
    "category": "lens",
    "subCategory": "prime"
  },
  {
    "title": "Canon EF-S 55-250mm f/4-5.6 IS STM",
    "slug": "canon-55-250",
    "company": "Canon",
    "price": 18999,
    "image": "/Assets/cannon 55 2502.png",
    "sliderImages": [
      "/Assets/cannon 55 2502.png",
      "/Assets/cannon 55 2504.png",
      "/Assets/cannon 55 2503.png",
      "/Assets/cannon 55 2505.png",
      "/Assets/cannon 55 2502.png"
    ],
    "description": "A lightweight telephoto zoom lens designed for capturing distant subjects with sharpness and smooth stabilization.",
    "features": [
      "55–250mm telephoto zoom for capturing distant subjects.",
      "Optical Image Stabilization reduces blur.",
      "STM motor enables smooth and silent autofocus.",
      "Compact and lightweight design.",
      "Ideal for wildlife and sports photography.",
      "Sharp image quality with good contrast."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "55–250mm"
      },
      {
        "key": "Aperture",
        "value": "f/4–5.6"
      },
      {
        "key": "Lens Type",
        "value": "Zoom"
      },
      {
        "key": "Mount",
        "value": "Canon EF-S"
      },
      {
        "key": "Stabilization",
        "value": "Yes"
      },
      {
        "key": "Weight",
        "value": "375g"
      }
    ],
    "detailSections": [
      {
        "title": "Telephoto Zoom",
        "description": "The 55–250mm range allows you to capture distant subjects with clarity.",
        "image": "/Assets/cannon 55 2501.png"
      },
      {
        "title": "Image Stabilization",
        "description": "Reduces blur and improves sharpness during handheld shooting.",
        "image": "/Assets/cannon 55 2503.png"
      },
      {
        "title": "STM Autofocus",
        "description": "Provides smooth and quiet focusing for photos and videos.",
        "image": "/Assets/cannon 55 2504.png"
      }
    ],
    "designSections": [
      {
        "title": "Compact Build",
        "description": "Lightweight design makes it easy to carry for travel and outdoor photography.",
        "image": "/Assets/cannon 55 2505.png"
      },
      {
        "title": "Durable Construction",
        "description": "Built with quality materials to ensure long-lasting performance.",
        "image": "/Assets/cannon 55 2501.png"
      }
    ],
    "connectivity": [
      {
        "title": "Wildlife",
        "description": "Ideal for capturing distant animals and nature scenes."
      },
      {
        "title": "Sports",
        "description": "Perfect for shooting fast-moving subjects from a distance."
      },
      {
        "title": "Portrait",
        "description": "Provides background blur and subject isolation."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/N5wZAvUmPUA",
    "category": "lens",
    "subCategory": "prime"
  },
  {
    "title": "Canon EF 70-200mm f/2.8L IS III USM",
    "slug": "canon-70-200",
    "company": "Canon",
    "price": 209999,
    "image": "/Assets/canon70-200-1.png",
    "sliderImages": [
      "/Assets/canon70-200-1.png",
      "/Assets/canon70-200-2.png",
      "/Assets/canon70-200-3.png",
      "/Assets/canon70-200-4.png",
      "/Assets/canon70-200-5.png"
    ],
    "description": "A professional telephoto zoom lens with constant f/2.8 aperture, ideal for sports, wildlife, and portrait photography.",
    "features": [
      "70–200mm telephoto zoom range.",
      "Constant f/2.8 aperture for low light.",
      "Advanced Image Stabilization.",
      "Fast USM autofocus.",
      "L-series professional build quality."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "70–200mm"
      },
      {
        "key": "Aperture",
        "value": "f/2.8"
      },
      {
        "key": "Mount",
        "value": "Canon EF"
      },
      {
        "key": "Stabilization",
        "value": "Yes"
      },
      {
        "key": "Weight",
        "value": "1480g"
      }
    ],
    "detailSections": [
      {
        "title": "Telephoto Zoom",
        "description": "Capture distant subjects with stunning clarity and detail.",
        "image": "/Assets/canon70-200-2.png"
      },
      {
        "title": "Image Stabilization",
        "description": "Reduces blur and ensures sharp handheld shots.",
        "image": "/Assets/canon70-200-3.png"
      },
      {
        "title": "Fast Autofocus",
        "description": "USM motor delivers quick and silent focusing.",
        "image": "/Assets/canon70-200-3.png"
      }
    ],
    "designSections": [
      {
        "title": "Professional Build",
        "description": "Durable L-series construction for demanding environments.",
        "image": "/Assets/canon70-200-4.png"
      },
      {
        "title": "Weather Sealing",
        "description": "Resistant to dust and moisture for outdoor use.",
        "image": "/Assets/canon70-200-2.png"
      }
    ],
    "connectivity": [
      {
        "title": "Sports",
        "description": "Perfect for capturing fast-moving subjects with precision and clarity."
      },
      {
        "title": "Wildlife",
        "description": "Ideal for shooting distant animals without disturbing them."
      },
      {
        "title": "Portrait",
        "description": "Produces beautiful background blur with sharp subject focus."
      },
      {
        "title": "Events",
        "description": "Great for weddings and events with versatile zoom range."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/oonKsqpH0OE",
    "category": "lens",
    "subCategory": "zoom"
  },
  {
    "title": "Canon EF 70-300mm f/4-5.6L IS USM",
    "slug": "canon-70-300",
    "company": "Canon",
    "price": 125999,
    "image": "/Assets/canon70-300-2.png",
    "sliderImages": [
      "/Assets/canon70-300-1.png",
      "/Assets/canon70-300-4.png",
      "/Assets/canon70-300-3.png",
      "/Assets/canon70-300-4.png",
      "/Assets/canon70-300-2.png"
    ],
    "description": "A compact L-series telephoto zoom lens delivering excellent performance, portability, and image quality.",
    "features": [
      "70–300mm versatile telephoto zoom.",
      "Image Stabilization for sharper images.",
      "Fast and precise USM autofocus.",
      "L-series professional build quality.",
      "Dust and moisture resistant design."
    ],
    "specifications": [
      {
        "key": "Focal Length",
        "value": "70–300mm"
      },
      {
        "key": "Aperture",
        "value": "f/4–5.6"
      },
      {
        "key": "Mount",
        "value": "Canon EF"
      },
      {
        "key": "Stabilization",
        "value": "Yes"
      },
      {
        "key": "Weight",
        "value": "1050g"
      }
    ],
    "detailSections": [
      {
        "title": "Telephoto Zoom",
        "description": "Capture distant subjects with clarity and flexibility.",
        "image": "/Assets/canon70-300-4.png"
      },
      {
        "title": "Image Stabilization",
        "description": "Reduces shake for sharper handheld shots.",
        "image": "/Assets/canon70-300-3.png"
      },
      {
        "title": "Fast Autofocus",
        "description": "USM ensures quick and accurate subject tracking.",
        "image": "/Assets/canon70-300-4.png"
      }
    ],
    "designSections": [
      {
        "title": "Compact Build",
        "description": "Portable and lightweight for everyday use.",
        "image": "/Assets/canon70-300-3.png"
      },
      {
        "title": "Weather Sealing",
        "description": "Designed to withstand dust and moisture.",
        "image": "/Assets/canon70-300-2.png"
      }
    ],
    "connectivity": [
      {
        "title": "Travel",
        "description": "Compact zoom lens ideal for travel photography."
      },
      {
        "title": "Wildlife",
        "description": "Capture distant subjects with ease."
      },
      {
        "title": "Sports",
        "description": "Great for fast-moving subjects."
      }
    ],
    "youtubeVideo": "https://www.youtube.com/embed/-7QkqSdenyI",
    "category": "lens",
    "subCategory": "zoom"
  }
];